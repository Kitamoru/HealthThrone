...
/** Офисные слова запрещены по промпту */
const FORBIDDEN_WORDS = [
  'задача', 'проект', 'дедлайн', 'коллеги',
  'продуктивность', 'эффективность', 'мотивация',
  'митинг', 'встреча', 'менеджер', 'офис',
];
...
export function validateQuestions(questions: Question[]): ValidationResult {
  ...
  for (const q of questions) {
    ...
    // Должен быть вопросом (заканчивается на ?)
    if (!q.text.trim().endsWith('?')) {
      return { ok: false, reason: `Вопрос id=${q.id} не заканчивается на "?": "${q.text}"` };
    }

    // Обращение на "вы" (хотя бы одно из ключевых слов)
    const hasPoliteForm = /\bвы\b|\bвам\b|\bваш|\bвас\b/i.test(q.text);
    if (!hasPoliteForm) {
      return { ok: false, reason: `Вопрос id=${q.id} не содержит обращения на "вы"` };
    }
  }
