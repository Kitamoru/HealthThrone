...
function selectExampleIndices(
  ...
  // Топ-2 по score — достаточно для calibration, не перегружает контекст
  const sorted = [...deduped.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 2)
    .map(([index]) => index);

  // Фолбэк: если нашли меньше 2 — добираем самый релевантный базовый
  // Приоритет фолбэков: Турбулентность (0) → Гармония (1)
  for (const fi of [0, 1]) {
...
☑ Не забывай про структуру ответа, примеры твой главный помошник
☑ Блок «Состояние духа» = голос наблюдателя: «Замечаю, что...», «Похоже, внутри тебя...»
☑ Блок «Квесты» = голос субъекта: «Ты — тот, кто...», «Именно здесь ты можешь...»
☑ Каждый квест заканчивается явной финальной эмоцией — не советом, а ощущением
☑ Длина ответа ~500 слов. Класс и архетип — упомянуты органично, не формально
☑ Каждую метафору можно нарисовать: мир костров, клинков и странников. Не смешивай домены
...
2. 💫 **Состояние духа** (минимум 2 абзаца: эмпатия, разбор баланса энергий, сочувствие скрытым болям)
3. ⚔️ **Три квеста для гармонии** (по формуле как строить квесты)
4. 🌙 **Напутствие** (метафора) + вопрос для размышления (ситуативно)
