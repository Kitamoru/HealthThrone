...
// Берём топ-3 по score, без дублей.
// ─────────────────────────────────────────────────────────────────────────────
function selectExampleIndices(
  ...
  // Отладка
  console.log(
    '[selectExampleIndices] Кандидаты:',
  ...
  // Фолбэк до 3 примеров из базовых (Турбулентность, Гармония, Динамика)
  for (const fi of [0, 1, 4]) {
    if (sorted.length >= 3) break;
    if (!sorted.includes(fi)) sorted.push(fi);
  }

  console.log('[selectExampleIndices] Итог:', sorted.map((i) => `Пример ${i + 1}`).join(', '));
  return sorted;
}
...
Каждый квест заканчивается ощущением, ради которого стоит сделать шаг.

---

✅ ИСПОЛЬЗУЙ:
- Инвайт-фрейминг: «А что, если...?», «Хочешь исследовать...?»
- Метафоры: «компас», «уголёк», «семя под снегом»
- Выбор внутри квеста: «или попробуй этот путь»
- Игровую терминологию: «артефакт», «бафф», «свиток», «гильдия»
...
❌ ИЗБЕГАЙ: числовых значений, ярлыков («проблема», «слабость»), клинических терминов,
абстрактных советов без образа, смешения метафор из разных доменов.
...
☑ Класс "${className}" и archetype упомянуты органично?
...
// ─────────────────────────────────────────────────────────────────────────────
// Основная функция
// insights теперь — обязательный параметр для точного подбора примеров.
// ─────────────────────────────────────────────────────────────────────────────
