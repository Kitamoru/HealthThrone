...
type CharacterClass = 'Достигатор' | 'Исследователь' | 'Социализатор' | 'Убийца';
type Answer = 'a' | 'b' | 'c' | 'd';
...
const CLASS_DESCRIPTIONS: Record<Role, Record<CharacterClass, string>> = {
  'Разработчик': {
...
const Onboarding = ({ onComplete }: { onComplete: () => void }) => {
  ...
  const handleAnswer = (answer: Answer) => {
    ...
    if (currentQuestion < QUESTIONS.length - 1) {
      ...
      calculateClass(newAnswers);
      setStep('result');
    }
  };

  // Расчет класса персонажа
  const calculateClass = (answers: Answer[]) => {
    const scores = {
      'Достигатор': 0,
      'Исследователь': 0,
      'Социализатор': 0,
    ...
    answers.forEach(answer => {
      switch(answer) {
        case 'a': scores['Достигатор']++; break;
        case 'b': scores['Исследователь']++; break;
        case 'c': scores['Социализатор']++; break;
        case 'd': scores['Убийца']++; break;
      }
    });

    const maxScore = Math.max(...Object.values(scores));
    const finalClass = (Object.keys(scores) as CharacterClass[]).find(
      key => scores[key] === maxScore
    ) || 'Достигатор';

    setCharacterClass(finalClass);
  };

  // Сохранение результата
  const saveResult = async () => {
    if (!selectedRole || !characterClass) return;
  ...
  return (
    <div className="min-h-screen bg-dungeon bg-cover text-white p-4">
      ...
      {step === 'result' && selectedRole && characterClass && (
